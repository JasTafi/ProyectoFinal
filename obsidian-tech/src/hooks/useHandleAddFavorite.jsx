	...
	const handleAddFavorites = async (id) => {
	  try {
		if (islogged) {
		  ...
		  Notification({ message: "Debes iniciar sesión para agregar a favoritos", type: "error" });
		}
	  } catch (err) {
		console.log(err);
		Notification({ message: "No se pudo agregar el producto a favoritos", type: "error" });
	  }
	};
